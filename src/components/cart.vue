            ...
            <div class="cart-table">
              <div class="cart-group">
                ...
                <div class="cart-top-items"
                     v-for="cargoods in getcardata"
                     :key="cargoods.sku_id">
                  <div class="cart-items">
                    <div class="items-choose">
                      <span class="blue-checkbox-new"
                            :class="{'checkbox-on':cargoods.checked}"
                            @click="changeable(cargoods)"><a></a></span>
                    </div>
                    <div class="items-thumb">
                      <img :src="cargoods.ali_image">
                      <a href="javascript:;"
                         target="_blank"></a>
                    </div>
                    <div class="name hide-row">
                      <div class="name-table">
                        <a href="javascript:;"
                           target="_blank">{{ cargoods.title }}</a>
                        <ul class="attribute">
                          <li>{{ cargoods.spec_json.show_name }}</li>
                        </ul>
                      </div>
                    </div>
                    <div class="operation">
                      <a class="items-delete-btn"
                         @click="delshop(cargoods.sku_id)"></a>
                    </div>
                    <div class="subtotal">¥ {{cargoods.count*cargoods.price}}.00</div>
                    <div class="item-cols-num">
                      <div class="select js-select-quantity">
                        <span class="down"
                              :class="{'down-disabled':cargoods.count<=1}"
                              @click="reduce(cargoods)">-</span>
                        <span class="num">
                          <input type="text"
                                 style="display: inline-block;"
                                 :value="cargoods.count">
                          <ul>
                            <li>1</li>
                            <li>2</li>
                          </ul>
                        ...
                        <span class="up"
                              :class="{'up-disabled':cargoods.count>=cargoods.limit_num}"
                              @click="increase(cargoods)">+</span>

                      </div>
                    </div>
                    <div class="price">¥ {{ cargoods.price }}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <div class="choose-all js-choose-all">
                <span class="blue-checkbox-new"
                      ...
                      @click="totalchecked(allchecked)"><a></a></span>
                全选
              </div>
              <div class="delete-choose-goods"
                   @click="delcheckedgoods()">删除选中的商品</div>
            </div>
          </div>
          <div class="shipping">
            ...
            <span class="jianguo-blue-main-btn big-main-btn js-checkout"
                  :class="{'disabled-btn':checkedprice<=0}">
              <router-link to="/checkout">现在去结算</router-link>
            </span>
...
export default {
  ...
  computed: {
    ...
    allchecked () {
      return this.$store.getters.totalcheckedgoods;
    },
    checkedcount () {
  ...
  methods: {
    increase (data) {
      this.$store.commit('increase', data)
    },
    reduce (data) {
      this.$store.commit('reduce', data)
    },
    delshop (id) {
      this.$store.commit("delshops", id)
    },
    changeable (data) {
      this.$store.commit('checked', data)
    },
    totalchecked (totalchecked) {
      this.$store.commit('allcheckedgoods', totalchecked)
    },
    delcheckedgoods () {
