          ...
          <div class="invoice-detail"> 发票抬头：
            <div class="radio-box">
              <label>
                ...
                <span class="blue-radio"
                      :class="{'blue-radio-on':invoicestate }"
                      @click="person()"><a></a></span> 个人
              </label>
              <label>
                <input type="radio"
                       class="hide">
            ...
            <div class="module-form-row form-item fn-hide js-invoice-title"
              ...
              <div class="module-form-item-wrapper no-icon small-item"
                   v-show="!invoicestate">
                <i v-show="!invoicedata">请填写公司发票抬头</i>
                <input type="text"
                       class="js-verify"
                       v-model="invoicedata">
              </div>
      ...
      <div class="gray-box">
          ...
          <div class="cart-table">
            <div class="cart-group js-cart-group">
              <div class="cart-items"
                   v-for="checkeditem in checkedgoods "
                   :key="checkeditem.sku_id">
                <div class="items-thumb">
                  <a href="javascript:;"
                     target="_blank"><img :src="checkeditem.ali_image+'?x-oss-process=image/resize,w_80/quality,Q_100/format,webp'"></a>
                </div>
                <div class="name hide-row">
                  <div class="name-cell">
                    <a href="javascript:;"
                       :title="checkeditem.title + '(' +checkeditem.spec_json.show_name +')'"
                       target="_blank">{{ checkeditem.title + '(' +checkeditem.spec_json.show_name +')'}}</a>
                  </div>
                </div>
                <div class="subtotal">
                  <div class="subtotal-cell"> ¥ {{ checkeditem.count*checkeditem.price }}.00 </div>
                </div>
                <div class="goods-num">{{ checkeditem.count }}</div>
                <div class="price">¥ {{ checkeditem.price }}.00</div>
              </div>
            </div>
          </div>
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            ...
            <p> 运费： <span>+ ¥ {{ postage(checkedprice) }}.00</span> </p>
            <p class="discount-line js-discount-cash"> <em>现金券</em>： <span> - 0 </span> </p>
          </div>
        </div>
        <div class="box-inner">
          <div class="last-payment clear">
            <span class="jianguo-blue-main-btn big-main-btn payment-blue-bt fn-right js-checkout"> <a>提交订单</a> </span> <span class="prices fn-right">应付金额： <em>¥ {{ checkedprice+postage(checkedprice) }}.00</em></span>
          </div>
        </div>
      </div>
    </div>
    <address-pop v-if="popshow"
                 @fnc='popstate'></address-pop>
  </div>
</template>
<script>
...
export default {
  ...
  created () {
    this.receiveInfo.forEach((element, index) => {
      if (element.default) {
