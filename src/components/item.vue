      ...
      <div class="item-box">
        <div class="gallery-wrapper">
          <div class="gallery">
            <div class="thumbnail">
              <ul>
                <li :class="{'on':itemindex==index}"
                    @click="choose(index)"
                    v-for="(img,index) in getitemdata.ali_images"
                    :key="img"><img :src="img + '?x-oss-process=image/resize,w_54/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
            <div class="thumb">
              <ul>
                <li :class="{'on':itemindex==index}"
                    v-for="(img,index) in getitemdata.ali_images"
                    :key="img"><img :src="img + '?x-oss-process=image/resize,w_450/quality,Q_90/format,webp'"></li>
              </ul>
            </div>
          </div>
        </div>
        <div class="banner">
          <div class="sku-custom-title">
            <div class="params-price">
              <span><em>¥</em><i>{{ getitemdata.price }}</i></span>
            </div>
            <div class="params-info">
              <h4>{{ getitemdata.title }}</h4>
              <h6>{{ getitemdata.sub_title }}</h6>
            </div>
          </div>
            ...
            <div class="sku-dynamic-params clear">
              ...
              <ul class="params-colors">
                <li :class="{'cur':color.id == $route.query.itemId}"
                    v-for="(color) in getitemdata.sku_list"
                    :key="color.id">
                  <router-link :title="color.color"
                               :to="{path:'/item',query:{itemId:color.id}}"><img :src="'http://img01.smartisanos.cn/'+ color.image+'20X20.jpg'"></router-link>
                </li>
              </ul>
            </div>
            <div class="sku-dynamic-params clear">
              ...
              <div class="params-detail clear">
                <div class="item-num js-select-quantity">
                  ...
                  <span class="up"
                        :class="{'up-disabled':count>=getitemdata.limit_num}"
                        @click="increase(getitemdata.limit_num)">+</span>
                </div>
              </div>
            </div>
          </div>
          <div class="sku-status">
            <div class="cart-operation-wrapper clearfix">
              <span class="blue-title-btn js-add-cart"
                    @click="addShopList(getitemdata)"><a>加入购物车</a></span>
...
export default {
  ...
  methods: {
    ...
    addShopList (data) {
      var itemData = { info: data, count: this.count }
